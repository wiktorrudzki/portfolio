...
import { footerStyles } from "@styles/FooterC&S";
...
const Footer = () => {
  ...
  return (
    <footer className={footerStyles[`${themeState.theme}`].footer}>
      <h4 className={footerStyles[`${themeState.theme}`].h4}>@2022 Wiktor Rudzki</h4>
      <div className={footerStyles[`${themeState.theme}`].paddingMarginZero}>LOGO</div>
      <ul className={footerStyles[`${themeState.theme}`].icons}>
        <IconLink
          url="https://www.facebook.com/wiktor.rudzki.73"
          alt="Find me on Facebook!"
          icon={facebookIcon}
          style={`${footerStyles[`${themeState.theme}`].icon} ${footerStyles[`${themeState.theme}`].paddingMarginZero}`}
        />
        <IconLink
